...
#define AEAD_CIPHERTEXT_BUF_SIZE_PLUS (AEAD_PLAINTEXT_BUF_SIZE + AEAD_MAC_SIZE + NUM_BUFFER_OVERFLOW_TEST_BYTES)                                           \
...
void exec_test_case_aead(void)
{
	...
	err_code = mbedtls_cipher_auth_decrypt_ext(
		&ctx, m_aead_nonce_buf, nonce_len, m_aead_ad_buf, ad_len,
		m_aead_input_buf, encrypt_operation_len, m_aead_output_buf, AEAD_CIPHERTEXT_BUF_SIZE_PLUS, &operation_len,
		mac_len);
